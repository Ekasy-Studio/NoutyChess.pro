...
import { Award, Ban, Crown, DoorClosed, Eye, Gamepad2, MessageCircle, Palette, Radio, RefreshCw, Save, Settings, ShieldCheck, Trophy, Users } from 'lucide-react';
...
type AdminData = {
  counters: { users: number; unique_visitors: number; sessions: number; online_players: number; active_rooms: number; games: number; banned: number };
...
type Tab = 'players' | 'online' | 'rooms' | 'games' | 'chat' | 'settings' | 'audit';
...
export function AdminDashboard({ adminName }: { adminName: string }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setData(body);
      setPixKey(String(body.settings?.value ?? ''));
    } catch (requestError) {
  ...
  const act = async (payload: Record<string, unknown>, reasonOverride?: string) => {
    ...
    if (!response.ok) {
      setError(body.error ?? 'A operação foi rejeitada.');
      return;
    }
    setReason('');
  ...
  return (
    <main className="admin-shell">
      <header className="admin-header">
        ...
        <Button variant="secondary" onClick={() => void load()} disabled={loading}><RefreshCw className={loading ? 'spin' : ''} /> Atualizar</Button>
      </header>

      <section className="admin-hero">
        <div><span><ShieldCheck /> PAINEL SEGURO</span><h1>Central de comando</h1><p>Jogadores, partidas, salas e moderação com registro de cada ação.</p></div>
        <div className="admin-live"><i /> Sistema operacional</div>
      </section>
      ...
      <section className="admin-metrics">
        ...
        <Metric icon={<Gamepad2 />} label="Salas ativas" value={data?.counters?.active_rooms ?? 0} />
      ...
      <section className="admin-workspace">
        <nav className="admin-tabs" aria-label="Áreas administrativas">
          {([['players', 'Jogadores'], ['online', 'Online agora'], ['rooms', 'Salas'], ['games', 'Partidas'], ['chat', 'Chat'], ['settings', 'Configurações'], ['audit', 'Auditoria']] as const).map(([id, label]) => (
        ...
        {loading ? <div className="admin-loading"><RefreshCw className="spin" /> Carregando dados protegidos…</div> : (
          <>
            {tab === 'players' && (
              <div className="admin-grid-split">
                ...
                <aside className="admin-editor">
                  ...
                  {selectedUser ? (
                    <>
                      ...
                      <div className="admin-editor-actions">
                        ...
                        <Button onClick={() => void act({ action: 'grant-reward', userId: selectedUser.user_id, rewardCoins: Number(rewardCoins), rewardXp: Number(rewardXp), badge })}><Award /> Dar recompensa</Button>
                  ...
                  ) : <p>Selecione um jogador para ajustar ranking, recompensas ou moderação.</p>}
                </aside>
              </div>
            )}
            ...
            {tab === 'rooms' && <AdminTable headers={['Sala', 'Host', 'Convidado', 'Status', 'Último sinal', 'Ação']} rows={(data?.rooms ?? []).map((room) => [
              String(room.code), String(room.host_id).slice(0, 12), room.guest_id ? String(room.guest_id).slice(0, 12) : '—', String(room.status), formatDate(room.last_seen_at),
              <Button key="terminate" size="sm" variant="destructive" onClick={() => { const why = window.prompt('Motivo para encerrar a sala:'); if (why) void act({ action: 'terminate-room', roomCode: room.code }, why); }}><DoorClosed /> Encerrar</Button>,
            ])} />}
            ...
            {tab === 'chat' && <AdminTable headers={['#', 'Tipo', 'Jogador', 'Mensagem', 'Sala', 'Data', 'Ações']} rows={(data?.chat ?? []).map((message) => [String(message.id), message.scope === 'room' ? 'Sala' : 'Comunidade', `${String(message.avatar_emote)} ${String(message.display_name)}`, message.deleted_at ? '[removida]' : String(message.message), String(message.room_code ?? '—'), formatDate(message.created_at), <div className="admin-inline-actions" key="actions"><Button size="xs" variant="destructive" disabled={Boolean(message.deleted_at)} onClick={() => { const why = window.prompt('Motivo para remover a mensagem:'); if (why) void act({ action: 'delete-chat-message', messageId: message.id }, why); }}><Ban /> Remover</Button><Button size="xs" variant="secondary" onClick={() => { const why = window.prompt('Motivo para silenciar este jogador:'); if (why) void act({ action: 'mute-chat', userId: message.user_id, minutes: 60 }, why); }}><MessageCircle /> Silenciar 1h</Button></div>])} />}

            {tab === 'settings' && <div className="admin-settings-card"><span><Settings /></span><div><small>APOIE A EKASY</small><h2>Chave Pix pública</h2><p>Deixe vazio para manter a página de apoio em modo “em breve”.</p><label>Chave Pix<input value={pixKey} onChange={(event) => setPixKey(event.target.value.slice(0, 140))} maxLength={140} /></label><label>Motivo da alteração<textarea value={reason} onChange={(event) => setReason(event.target.value.slice(0, 240))} maxLength={240} /></label><Button onClick={() => void act({ action: 'update-pix', pixKey })}><Save /> Salvar configuração</Button></div></div>}
