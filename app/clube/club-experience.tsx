...
export function ClubExperience({ membership, signInPath }: { membership: { active: boolean; until: number | null; name: string } | null; signInPath: string }) {
  ...
  return (
    <main className="club-shell">
      <header className="simple-page-header"><Link className="brand" href="/"><span className="brand-mark">♘</span><span><strong>NoutyChess</strong><small>Jogue melhor</small></span></Link><Link href="/">Voltar ao jogo</Link></header>
      <section className="club-hero">
        <div className="club-orbit"><i /><span><Crown /></span></div>
        <div><span className="club-kicker"><Sparkles /> CLUBE LENDÁRIO</span><h1>Não é só jogar.<br />É deixar sua assinatura.</h1><p>Uma experiência de membro feita para quem quer pertencer à história do NoutyChess, sem comprar vantagem no ranking.</p>
          {membership?.active
            ? <div className="member-active"><ShieldCheck /><span><strong>{membership.name}, seu Clube está ativo.</strong><small>Vá ao perfil e equipe seus benefícios do Clube.</small></span></div>
            : membership
              ? <Button className="club-cta" onClick={() => void requestMembership()} disabled={busy || requested}>{requested ? <><Star /> Solicitação enviada</> : <><Crown /> Solicitar Clube Lendário</>}</Button>
              : <a className="club-cta club-cta-link" href={signInPath} target="_top"><LogIn /> Entrar e solicitar acesso</a>}
          {error && <div className="network-error" role="alert">{error}</div>}
          <small className="club-honesty">A solicitação não gera cobrança. O acesso só é ativado depois da aprovação administrativa.</small>
        </div>
      </section>
      ...
      <section className="club-benefits">
        ...
        <article><Gem /><h3>Legado fundador</h3><p>Itens especiais podem ser concedidos em eventos e campanhas oficiais.</p></article>
      </section>
      ...
      <footer className="simple-page-footer">Feito pela <strong>Ekasy Studio</strong>.</footer>
    </main>
  );
}
